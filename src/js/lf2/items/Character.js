...
import { Body } from '../frame/Body.js';
import { Interaction } from '../frame/Interaction.js';
...
const STAND_FRAME_RANGE = { min: 0, max: 4 };
Object.freeze(STAND_FRAME_RANGE);

const WALK_FRAME_RANGE = { min: 5, max: 8 };
Object.freeze(WALK_FRAME_RANGE);

const RUN_FRAME_RANGE = { min: 9, max: 11 };
Object.freeze(RUN_FRAME_RANGE);

const FALLING1_FRAME_RANGE = { min: 180, max: 185 };
Object.freeze(FALLING1_FRAME_RANGE);

const FALLING2_FRAME_RANGE = { min: 186, max: 191 };
Object.freeze(FALLING2_FRAME_RANGE);

const HIDE_FRAME_RANGE = { min: 1100, max: 1299 };
...
const RECOVERY = {
    HP: { value: 10, interval: 5000 },
    MP: { value: (player) => 1 + ((player.hp / 100) | 0), interval: 500 },
    FALL: { value: -0.45 },
    BDEFEND: { value: -0.5 },
};
...
export class Character extends GameItem {
    ...
    _getNextFrameId() {
        ...
        if (fc) {
            if (
                next !== 0 &&
                next.inRange(STAND_FRAME_RANGE.min, STAND_FRAME_RANGE.max) && (IS_ARR_ONLY)
            ) {
                next = 999;
            }
        }
    ...
    update() {
        ...
        if (frameKind === 80 || state === 9995) {
            ...
            this.obj = lf2.GameObjectPool.get(newCharId);
